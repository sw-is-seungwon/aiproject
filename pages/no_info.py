...
# 💡 버그 2 해결: 사용자가 사이드바를 조작해 모드를 직접 전환하면 상태값을 싹 강제 포맷팅합니다.
    ...
    if search_mode == "너비 우선 탐색 (BFS)":
        ...
        if st.button("⏪ 이 분기 취소하고 다른 너비 탐색하기 (뒤로 가기)"):
            if len(st.session_state.bfs_history) > 1:
                ...
                st.session_state.bfs_next_level_parent = None
                st.rerun()  # 💡 버그 1 해결: st.rerun() 최신 표준 변경
    else:
...
# 💡 버그 3 해결: fixedsize='true'와 가로폭 width='1.4'를 세팅해 상자 크기가 제각각 일그러지는 현상을 완벽 차단
